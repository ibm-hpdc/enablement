...
parser.add_argument('--token', '-t', help='Token used to approve the policy')
...
if args.token:
   ...
   token = getpass.getpass(prompt='Token for approving the policy: ')
...
print('***** Approve a policy')

url = f"https://{hpdchost}:19443/policies/{policy}/approve"
body = {"token": token}
response = s.post(url, headers=api_headers, json=body, verify=False)
print ('***** Response code {}'.format(response.status_code))
if response.status_code != 200:
