...
parser.add_argument('--labelpw', '-lp', help='Password for the password label')
...
if args.labelpw:
   labelpw = args.labelpw
else:
   labelpw = getpass.getpass(prompt='Password for the label: ')
...
print('***** Create a password label')

url = f'https://{hpdchost}:19443/passwords/{label}'
body = {"password":labelpw}
response = s.post(url, headers=api_headers, json=body, verify=False)
print ('***** Response code {}'.format(response.status_code))
if response.status_code != 200:
   print('***** Request failed')
